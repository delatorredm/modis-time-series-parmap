...
coord <- c(14.5995, 120.9842) # Coordinates south of Mount Kilimanjaro
product <- "MOD13Q1"
...
period <- data.frame(lat = coord[1], 
                     long = coord[2], 
                     start.date = 2001, 
                     end.date = 2016, 
                     id = 1)
# To download the pixels
MODISSubsets(LoadDat = period,
            Products = product,
             Bands = bands,
             Size = pixel,
             SaveDir = savedir,
...
library(MODIS)
dates <- as.POSIXct(as.Date(c("01/05/2016", "31/05/2016"), format = "%d/%m/%Y") )
...
runGdal(product = product, 
        #outProj = 'EPSG:32651', 
        begin = dates2$beginDOY, end = dates2$endDOY, tileH = h, tileV = v)
# Per Default the data will be stored in
# ~homefolder/MODIS_ARC/PROCESSED
# After download you can stack the processed TIFS
vi <- preStack(path="~/MODIS_ARC/PROCESSED/MOD13Q1.005_20140810192530/", 
               pattern="*_EVI.tif$")
s <- stack(vi)
...
sp <- SpatialPoints(coords = cbind(coord[2], coord[1]), 
...
#+init=epsg:32651 +proj=utm +zone=51 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0